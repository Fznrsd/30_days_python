...
def get_mean_weight(cat_dict):
    ...
    for cat in cat_dict:
        ...
        mean_list.append(int(lower_weight))
        mean_list.append(int(upper_weight))

    # mean = sum of parts / length
    sum_list = sum(mean_list)
    cat_mean = sum_list / len(mean_list)
    return cat_mean
...
def get_mean_life(cat_dict):
    ...
    for cat in cat_dict:
        ...
        mean_list.append(int(lower_life))
        mean_list.append(int(upper_life))
...
def get_largest_countries(country_dict, ret_count=-1):
    ...
    return sorted(pop_dict.items(), key=lambda x: x[1], reverse=True)[:ret_count]
...
# Send a GET request to the UCI page
...
# Find the content you want to extract, such as the page title
page_title = soup.title.text
print("Page Title:", page_title)

# You can explore the HTML structure and extract other information as needed
...
# print(translated_text)  # Output: 123
